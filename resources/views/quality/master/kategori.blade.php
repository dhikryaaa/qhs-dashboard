...
@section('page-title')
<h1 class="page-title-header"><span class="breadcrumb-parent">Master</span> / <span class="breadcrumb-active">Kategori</span></h1>
@endsection

@section('content')
<style>
    /* ========== MASTER KATEGORI PAGE STYLES ========== */
...
<div class="content-card">
    <div class="table-header">
        <h2 class="table-title">List Kategori</h2>
...
<div id="createRoleModal" class="modal-overlay">
    <div class="modal-window">
            ...
            <form id="kategoriForm">
                <div class="form-row">
                    <div class="form-group-half">
                        ...
                        <input type="text" id="kodeKategoriInput" class="form-input-half" placeholder="Masukkan kode kategori">
                    </div>
                    <div class="form-group-half">
                        <label class="form-label">Nama Kategori</label>
                        <input type="text" id="namaKategoriInput" class="form-input-half" placeholder="Masukkan nama kategori">
                    </div>
                </div>
            </form>
        </div>
        <div class="modal-footer">
            ...
            <button class="btn-save" onclick="saveKategori()">Save</button>
        </div>
    </div>
</div>

<!-- Modal Delete Kategori -->
<div id="deleteKategoriModal" class="modal-delete-overlay">
...
<script>
    ...
    function renderTable(data) {
        ...
        data.data.forEach((kategori, index) => {
            ...
            row.dataset.id = kategori.kode;
            
            const isActive = kategori.aktif === 'Y' || kategori.aktif === true;
            ...
            // Column 2: Kode Kategori
            const tdKode = document.createElement('td');
            tdKode.textContent = kategori.kode;
            row.appendChild(tdKode);
            
            // Column 3: Nama Kategori
            const tdNama = document.createElement('td');
            tdNama.textContent = kategori.nama;
            ...
            toggleDiv.dataset.status = kategori.aktif;
            toggleDiv.dataset.kategoriId = kategori.kode;
            toggleDiv.onclick = function() {
                toggleStatus(this, kategori.kode);
            };
            ...
            editSpan.onclick = function() {
                openEditModal(kategori.kode, kategori.kode, kategori.nama);
            };
            ...
            deleteSpan.onclick = function() {
                openDeleteModal(kategori.kode, kategori.nama);
            };
    ...
    function handlePageChange() {
        ...
        currentPage = 1;
        loadKategoris(currentPage);
    }
    
    function nextPage() {
        loadKategoris(currentPage + 1);
    }
    ...
    function openModal() {
        ...
        document.getElementById('createRoleModal').querySelector('.modal-title').textContent = 'Tambah Kategori';
        document.getElementById('createRoleModal').classList.add('active');
    }
    
    function openEditModal(id, kode, nama) {
        editingKategoriId = id;
        document.getElementById('kodeKategoriInput').value = kode;
        document.getElementById('namaKategoriInput').value = nama;
        document.getElementById('createRoleModal').querySelector('.modal-title').textContent = 'Edit Kategori';
        document.getElementById('createRoleModal').classList.add('active');
    }
    ...
    function saveKategori() {
        const kode = document.getElementById('kodeKategoriInput').value.trim();
        const nama = document.getElementById('namaKategoriInput').value.trim();
        
        if (!kode || !nama) {
            alert('Kode kategori dan nama kategori harus diisi');
            return;
        }
        
        if (editingKategoriId) {
            // Update
            updateKategori(editingKategoriId, kode, nama);
        } else {
            // Create
            createKategori(kode, nama);
        }
    }
    
    function createKategori(kode, nama) {
        fetch(API_BASE, {
            method: 'POST',
            headers: {
    ...
    function updateKategori(id, kode, nama) {
        fetch(`${API_BASE}/${id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
    ...
    let kategoriNameToDelete = null;
    
    function openDeleteModal(kategoriId, kategoriName) {
        kategoriToDelete = kategoriId;
        kategoriNameToDelete = kategoriName;
        document.getElementById('deleteKategoriModal').classList.add('active');
    }
    
    function closeDeleteModal() {
        document.getElementById('deleteKategoriModal').classList.remove('active');
        kategoriToDelete = null;
        kategoriNameToDelete = null;
    }
    
    function confirmDelete() {
        if (kategoriToDelete) {
            fetch(`${API_BASE}/${kategoriToDelete}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content || ''
            ...
            .then(data => {
                alert(`Kategori "${kategoriNameToDelete}" berhasil dihapus`);
                closeDeleteModal();
                loadKategoris(currentPage);
            })
            .catch(error => {
